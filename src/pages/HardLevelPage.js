...
const HardLevelPage = () => {
    ...
    const data_arr = verb_data
        .map(([ff, sf, tf]) => ({ ff, sf, tf }));
    const verb_obj = { ...data_arr }
    const verb_no = Math.floor(Math.random() * verb_data.length)

    const ff = verb_obj[verb_no].ff
    const sf = verb_obj[verb_no].sf
    const tf = verb_obj[verb_no].tf

    return (
        <HOCMain>
            <CompleteAnsComponent level='hard' firstform={ff} secondform={sf} thirdform={tf} />
        </HOCMain>
    )
}
